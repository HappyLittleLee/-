...
#加载训练数据
df = pd.DataFrame.from_records(read_json('train.json'))
...
trainer.train()
trainer.save_model(f'./best_ckpt/')#tensorflow的模型保存文件.ckpt


#inference
model = BertForSequenceClassification.from_pretrained('./best_ckpt/', num_labels=36)
model = model.to('cuda')
tokenizer = BertTokenizerFast.from_pretrained('mengzi-bert-base.bin')

df_test = pd.DataFrame.from_records(read_json('testA.json'))
df_test['input_string'] = df_test.apply(
    lambda x: f"这份专利的标题为：《{x.title}》，由“{x.assignee}”公司申请，详细说明如下：{x.abstract}", axis=1)
